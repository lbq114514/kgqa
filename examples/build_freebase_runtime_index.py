"""Build runtime retrieval indexes for one Freebase SQLite truth database."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Build KGQA runtime indexes from one Freebase SQLite database.")
    parser.add_argument("--db-path", required=True, help="Path to the source SQLite database")
    parser.add_argument("--index-dir", required=True, help="Output directory for runtime index files")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite an existing index directory database.")
    parser.add_argument(
...
def main() -> None:
    """Build the configured runtime index."""
    args = parse_args()
