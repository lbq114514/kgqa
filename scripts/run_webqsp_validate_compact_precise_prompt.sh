#!/usr/bin/env bash

set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$PROJECT_ROOT"

CONDA_BASE="${CONDA_BASE:-/home/ubuntu/anaconda3}"
CONDA_ENV_NAME="${CONDA_ENV_NAME:-kgqa}"
source "${CONDA_BASE}/etc/profile.d/conda.sh"
conda activate "${CONDA_ENV_NAME}"

resolve_project_path() {
  local path_value="$1"
  if [[ "$path_value" == /* ]]; then
    printf '%s\n' "$path_value"
  else
    printf '%s\n' "${PROJECT_ROOT}/${path_value}"
  fi
}

BASE_CONFIG="${BASE_CONFIG:-config.yaml}"
QA_CONCURRENCY="${QA_CONCURRENCY:-6}"
LIMIT="${LIMIT:-}"
RESUME="${RESUME:-0}"
SPLIT="${SPLIT:-validation}"
GRAPHAPI_ENABLED="${GRAPHAPI_ENABLED:-0}"
USE_Q_ENTITIES="${USE_Q_ENTITIES:-0}"

while [[ $# -gt 0 ]]; do
  case "$1" in
    --base-config)
      BASE_CONFIG="$2"
      shift 2
      ;;
    --qa-concurrency)
      QA_CONCURRENCY="$2"
      shift 2
      ;;
    --limit)
      LIMIT="$2"
      shift 2
      ;;
    --resume)
      RESUME=1
      shift
      ;;
    --split)
      SPLIT="$2"
      shift 2
      ;;
    --graphapi)
      GRAPHAPI_ENABLED=1
      shift
      ;;
    --use-q-entities)
      USE_Q_ENTITIES=1
      shift
      ;;
    --output-dir)
      OUTPUT_DIR="$2"
      shift 2
      ;;
    --log-file)
      LOG_FILE="$2"
      shift 2
      ;;
    --runtime-config)
      RUNTIME_CONFIG="$2"
      shift 2
      ;;
    *)
      echo "Unknown argument: $1" >&2
      exit 1
      ;;
  esac
done

RUN_NAME="webqsp_validate_compact_precise_prompt"
if [[ "$GRAPHAPI_ENABLED" == "1" ]]; then
  RUN_NAME="${RUN_NAME}_graphapi"
else
  RUN_NAME="${RUN_NAME}_no_graphapi"
fi

OUTPUT_DIR="${OUTPUT_DIR:-outputs/${RUN_NAME}}"
LOG_FILE="${LOG_FILE:-outputs/logs/${RUN_NAME}.log}"
RUNTIME_CONFIG_DIR="${RUNTIME_CONFIG_DIR:-outputs/runtime_configs}"
RUNTIME_CONFIG="${RUNTIME_CONFIG:-${RUNTIME_CONFIG_DIR}/${RUN_NAME}.yaml}"

OUTPUT_DIR="$(resolve_project_path "$OUTPUT_DIR")"
LOG_FILE="$(resolve_project_path "$LOG_FILE")"
RUNTIME_CONFIG="$(resolve_project_path "$RUNTIME_CONFIG")"

mkdir -p "$(dirname "$LOG_FILE")" "$(dirname "$RUNTIME_CONFIG")" "$OUTPUT_DIR"

python - "$BASE_CONFIG" "$RUNTIME_CONFIG" "$OUTPUT_DIR" "$GRAPHAPI_ENABLED" <<'PY'
import sys
from pathlib import Path
import yaml

base_config = Path(sys.argv[1])
runtime_config = Path(sys.argv[2])
output_dir = sys.argv[3]
graphapi_enabled = sys.argv[4] == "1"

config = yaml.safe_load(base_config.read_text(encoding="utf-8"))
config.setdefault("graphapi", {})["enabled"] = graphapi_enabled
if graphapi_enabled:
    config.setdefault("datasets", {}).setdefault("webqsp", {})["external_graph_only"] = True
config.setdefault("pruning", {})["compact_precise_path_prompt"] = True
config.setdefault("evaluation", {})["output_dir"] = output_dir
config["evaluation"]["sample_output_dir"] = "sample_predictions"
runtime_config.write_text(yaml.safe_dump(config, sort_keys=False, allow_unicode=True), encoding="utf-8")
PY

cmd=(
  python main.py
  --dataset webqsp
  --mode validate
  --split "$SPLIT"
  --qa-concurrency "$QA_CONCURRENCY"
  --config "$RUNTIME_CONFIG"
  --compact-precise-path-prompt
)

if [[ "$GRAPHAPI_ENABLED" == "1" ]]; then
  cmd+=(--graphapi-enabled)
fi

if [[ -n "$LIMIT" ]]; then
  cmd+=(--limit "$LIMIT")
fi

if [[ "$RESUME" == "1" ]]; then
  cmd+=(--resume)
fi

if [[ "$USE_Q_ENTITIES" == "1" ]]; then
  cmd+=(--use-q-entities)
fi

nohup "${cmd[@]}" >"$LOG_FILE" 2>&1 &
pid=$!

echo "Started ${RUN_NAME}"
echo "PID: $pid"
echo "Log: $LOG_FILE"
echo "Runtime config: $RUNTIME_CONFIG"
echo "Output dir: $OUTPUT_DIR"
echo "Conda env: $CONDA_ENV_NAME"
echo "Compact precise path prompt: 1"
echo "GraphAPI enabled: $GRAPHAPI_ENABLED"
