...
MODE_NAME="no_graphapi"
if [[ "$GRAPHAPI_ENABLED" == "1" ]]; then
  MODE_NAME="graphapi"
fi

RUN_NAME="webqsp_single_${MODE_NAME}_index${INDEX}"
...
python - "$BASE_CONFIG" "$RUNTIME_CONFIG" "$OUTPUT_DIR" "$GRAPHAPI_ENABLED" "$COMPACT_PRECISE_PATH_PROMPT" <<'PY'
import sys
...
compact_precise_path_prompt = sys.argv[5] == "1"
